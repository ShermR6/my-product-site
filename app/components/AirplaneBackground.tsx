...
export default function AirplaneBackground() {
  ...
  useEffect(() => {
    ...
    function makePlaneImage(size: number, opacity: number): HTMLCanvasElement {
      ...
      ox.closePath();
      ox.fill();
      ox.fillStyle = "rgba(14, 165, 233, 0.6)";
      ox.beginPath();
    ...
    function init() {
      planes = [];
      for (let i = 0; i < PLANE_COUNT; i++) planes.push(createPlane());
    }
      ...
      planes.forEach(p => {
        ...
        if (p.trail.length > 1) {
          for (let i = 1; i < p.trail.length; i++) {
            ...
            ctx!.strokeStyle = `rgba(14, 165, 233, ${p.opacity * frac * 0.45})`;
            ctx!.lineWidth = 0.8;
            ctx!.stroke();
          }
        }
